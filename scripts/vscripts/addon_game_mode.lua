...
function CAddonTemplateGameMode:OnThink()
	if GameRules:State_Get() == DOTA_GAMERULES_STATE_HERO_SELECTION and IsServer() then
		...
		if self.hero_selection_state == "INI" and self.game_mode == "LD" then
			...
			-- TODO signal all clients to show ban
		end
		if self.hero_selection_state == "BAN" and GameRules:GetDOTATime(true, true) > 30 then
			-- TODO add hero to whitelists
			self.game_mode = "PIC"
			picked_str_heroes = randomPickFromList(str_heroes, 20)
			picked_agi_heroes = randomPickFromList(agi_heroes, 20)
			picked_int_heroes = randomPickFromList(intellect_heroes, 20)
