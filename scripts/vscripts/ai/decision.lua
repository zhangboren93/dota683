...
function X:ExecuteMode(bot)
    if self.currentMode ~= self.prevMode then
        ...
        self.prevMode:OnEnd(bot)
        self.currentMode:OnStart(bot)
        self.prevMode = self.currentMode
    end

    self.currentMode:Think(bot)
end
...
function X:setHeroVar(bot, var, value)
    gHeroVar.SetVar(bot:GetPlayerID(), var, value)
end

function X:getHeroVar(bot, var)
...
function X:DoInit(bot)
    ...
    self:setHeroVar(bot, "Self", self)
    self:setHeroVar(bot, "LastCourierThink", -1000.0)
    self:setHeroVar(bot, "LastLevelUpThink", -1000.0)
    self:setHeroVar(bot, "TeamBuy", {})
    self:setHeroVar(bot, "DoDefendLane", {})
    bot.IsRetreating = false
    bot.RetreatHealthPerc = 0.25
    self:setHeroVar(bot, "Target", nil)
    self:setHeroVar(bot, "RoamTarget", nil)
    
    local botDifficulty = bot:GetDifficulty()
    if botDifficulty == DIFFICULTY_EASY then
        self:setHeroVar(bot, "AbilityDelay", 0.75)
    elseif botDifficulty == DIFFICULTY_MEDIUM then
        self:setHeroVar(bot, "AbilityDelay", 0.45)
    elseif botDifficulty == DIFFICULTY_HARD then
        self:setHeroVar(bot, "AbilityDelay", 0.0)
...
function X:Think(bot)
    ...
    if rt ~= nil and (rt:IsNull() or not rt:IsAlive()) then
        ...
        setHeroVar(bot, "RoamTarget", nil)
        bot:Action_ClearActions(true)
        return
    end
    
    -- level up abilities if time
    local checkLevel, newTime = utils.TimePassed(self:getHeroVar(bot, "LastLevelUpThink"), 2.0)
    if checkLevel then
        self:setHeroVar(bot, "LastLevelUpThink", newTime)
    ...
    self:getHeroVar(bot, "ItemPurchaseClass"):ItemPurchaseThink(bot)
    
    -- consider using items
    if item_usage.UseItems(bot) then return end
    ...
    if not utils.IsBusy(bot) then
        local bAbilityQueued = self:getHeroVar(bot, "AbilityUsageClass"):AbilityUsageThink(bot)
...
function X:AnalyzeLanes(bot, nLane)
    if utils.InTable(nLane, self:getHeroVar(bot, "CurLane")) then
        return
    end
    
    local currLane = self:getHeroVar(bot, "CurLane")
    local frontier = GetLaneFrontAmount(bot:GetTeam(), currLane, false)
    ...
    if #nLane > 1 then
        ...
        self:setHeroVar(bot, "CurLane", newLane)
    elseif #nLane == 1 then
        utils.myPrint("Switching to lane: ", nLane[1])
        self:setHeroVar(bot, "CurLane", nLane[1])
    else
        utils.myPrint("Switching to lane: ", LANE_MID)
        self:setHeroVar(bot, "CurLane", LANE_MID)
    end

    self:setHeroVar(bot, "LaningState", 1) -- 1 is LaningState.Moving
    return
end

function X:DoChangeLane(bot)
    local listBuildings = buildings_status.GetStandingBuildingIDs(utils.GetOtherTeam(bot))
    local nLane = {}
    ...
    if #nLane > 0 then
        return self:AnalyzeLanes(bot, nLane)
    end
