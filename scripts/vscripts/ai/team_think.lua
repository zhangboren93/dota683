...
function ConsiderGlyphUse()
    ...
    for i, building_id in pairs(vulnerableTowers) do
        ...
        if tower:GetHealth() < math.max(tower:GetMaxHealth()*0.15, 165) and TimeSinceDamagedByAnyHero(tower) < 3
            and TimeSinceDamagedByCreep(tower) < 3 then
...
function ConsiderTeamLaneDefense(bot)
    local lane, building, numEnemies = global_game_state.DetectEnemyPush(bot)

    local listAlly = bot:GetUnitList(UNIT_LIST_ALLIED_HEROES)

    for _, ally in pairs(listAlly) do
        if not ally:IsIllusion() and not ally:IsControllableByAnyPlayer() and gHeroVar.HasID(ally:GetPlayerID()) then
        ...
        if not ally:IsIllusion() and ally:IsBot() and ally:IsAlive() then -- TODO: is IsAlive needed?
            if gHeroVar.GetVar(ally:GetPlayerID(), "Self"):getCurrentMode():GetName() == "defendlane" then
                ...
                if ally:GetHealth()/ally:GetMaxHealth() >= 0.5 then
                    ...
                    local timeToReachBuilding = distFromBuilding/GetCurrentMovementSpeed(ally)
