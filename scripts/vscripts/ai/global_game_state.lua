...
function numEnemiesNearBuilding(bot, building)
    local num = 0
    local enemies = bot:GetUnitList(UNIT_LIST_ENEMY_HEROES)
    for _, enemy in pairs(enemies) do
        if nearBuilding(enemy:GetAbsOrigin(), buildings_status.GetLocation(bot:GetTeam(), building)) then
            num = num + 1
        end
    end
    return num
...
function DetectEnemyPushMid(bot)
    local building = buildings_status.GetVulnerableBuildingIDs(bot:GetTeam(), LANE_MID)[1]
...
function DetectEnemyPushTop(bot)
    local building = buildings_status.GetVulnerableBuildingIDs(bot:GetTeam(), LANE_TOP)[1]
...
function DetectEnemyPushBot(bot)
    local building = buildings_status.GetVulnerableBuildingIDs(bot:GetTeam(), LANE_BOT)[1]
...
function DetectEnemyPush(bot)
    local bUpdate, newTime = utils.TimePassed(lastPushCheck, 0.5)
    if bUpdate then
        local numMid, midBuilding = DetectEnemyPushMid(bot)
        local numTop, topBuilding = DetectEnemyPushTop(bot)
        local numBot, botBuilding = DetectEnemyPushBot(bot)
        if numMid > 0 then
...
function GlobalFightDetermination(bot)
    ...
    local listEnemies = bot:GetUnitList(UNIT_LIST_ENEMY_HEROES)
    
    for _, ally in pairs(listAllies) do
        if not ally:IsControllableByAnyPlayer() and not ally:IsIllusion() then
                    ...
                    if utils.ValidTarget(enemy) then
                        ...
                        if distance <= eyeRange then
                            ...
                            if enemy:GetHealth()/enemy:GetMaxHealth() > 0.25 then
                                ...
                                rawTotalEnemyPower = rawTotalEnemyPower + GetRawOffensivePower(enemy)
                            end
                            ...
                            for _, ally2 in pairs(listAllies) do
                                ...
                                if not ally2:IsIllusion() and ally2:GetPlayerID() ~= ally:GetPlayerID() then
                                    ...
                                    local allyTimeToReach = distToEnemy/GetCurrentMovementSpeed(ally2)
                                    
                                    if not ally2:IsControllableByAnyPlayer() then
                                        ...
                                        elseif globalAbility then
                                            totalNukeDmg = totalNukeDmg + GetActualIncomingDamage(enemy, globalDmg, globalDmgType)
                                            table.insert(globalAllies, {ally2, globalAbility, GetExtrapolatedLocation(enemy, globalDelay)})
                                        end
                                    else
                                        if allyTimeToReach <= 6.0 then
                                            totalNukeDmg = totalNukeDmg + GetOffensivePower(ally2)
                                        end
                                    end
                                end
                            end
                            ...
                            for _, nearEnemy in pairs(nearbyEnemyHeroes) do
                                if utils.ValidTarget(nearEnemy) and nearEnemy:GetPlayerID() ~= enemy:GetPlayerID() then
                                    local stun = GetStunDuration(nearEnemy, true)
                                    local slow = GetSlowDuration(nearEnemy, true)
