...
function drAbility:nukeDamage( bot, enemy )
    ...
    local stunTime = 0
    local slowTime = 0
    local engageDist = bot:Script_GetAttackRange() + bot:BoundingRadius2D()
    ...
    if abilityQ:IsFullyCastable() then
        if not magicImmune then
            ...
            local speedReduction = abilityQ:GetSpecialValueFor("frost_arrows_movement_speed")
            local numCasts = 1

            local dist = GetUnitToUnitDistance(bot, enemy)
            if dist < (bot:Script_GetAttackRange() + bot:BoundingRadius2D() + enemy:BoundingRadius2D()) then
                if GetCurrentMovementSpeed(bot) > (GetCurrentMovementSpeed(enemy) + speedReduction) then
                    numCasts = Min(Min(bot:GetLevel(), 6), math.floor(manaAvailable/12))
                else
                    local distToEscape = (bot:Script_GetAttackRange() + bot:BoundingRadius2D() + enemy:BoundingRadius2D()) - dist
                    local timeToEscape = distToEscape/(GetCurrentMovementSpeed(enemy) + speedReduction - GetCurrentMovementSpeed(bot))
                    numCasts = Min(math.floor(timeToEscape/bot:GetSecondsPerAttack(false)), math.floor(manaAvailable/12))
                end
            end
            ...
            for i = 1, numCasts, 1 do
                if manaCostQ <= manaAvailable then
                    ...
                    dmgTotal = dmgTotal + GetActualIncomingDamage(enemy, bot:GetAttackDamage(), DAMAGE_TYPE_PHYSICAL)
            ...
            dmgTotal = dmgTotal + Min(bot:GetLevel(),4)*GetActualIncomingDamage(enemy, bot:GetAttackDamage(), DAMAGE_TYPE_PHYSICAL)
            castTime = castTime + bot:GetAttackPoint()
        end
    end
...
function ConsiderW(bot)
    ...
    local WaveSpeed = abilityW:GetSpecialValueFor("wave_speed")
    ...
    if #tableNearbyAttackingAlliedHeroes >= 2 then
        local locationAoE = bot:FindAoELocation( true, true, bot:GetAbsOrigin(), CastRange, 250, 0, 0 )
...
function drAbility:AbilityUsageThink(bot)
    ...
    local castWDesire, castWLoc     = ConsiderW(bot)
    local castEDesire               = ConsiderE(bot)
