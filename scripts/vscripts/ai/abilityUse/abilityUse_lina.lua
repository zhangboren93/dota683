        ...
        if manaCostR <= manaAvailable then
            if bot:HasScepter() then
                ...
                if not magicImmune then
                    manaAvailable = manaAvailable - manaCostR
                    dmgTotal = dmgTotal + GetActualIncomingDamage(enemy, abilityR:GetAbilityDamage(), DAMAGE_TYPE_MAGICAL)
                    castTime = castTime + abilityR:GetCastPoint()
                    engageDist = Min(engageDist, abilityR:GetCastRange())
                    table.insert(comboQueue, abilityR)
                end
    ...
    if abilityQ:IsFullyCastable() then
        ...
        if manaCostQ <= manaAvailable then
            if not magicImmune then
                ...
                dmgTotal = dmgTotal + GetActualIncomingDamage(enemy, abilityQ:GetAbilityDamage(), DAMAGE_TYPE_MAGICAL)
    ...
    if abilityW:IsFullyCastable() then
        ...
        if manaCostW <= manaAvailable then
            if not magicImmune then
                ...
                dmgTotal = dmgTotal + GetActualIncomingDamage(enemy, abilityW:GetAbilityDamage(), DAMAGE_TYPE_MAGICAL)
                castTime = castTime + abilityW:GetCastPoint()
                stunTime = stunTime + abilityW:GetSpecialValueFor("light_strike_array_stun_duration")
                ...
                if euls then
                    local manaCostEuls = euls:GetManaCost(-1)
    ...
    if dist < engageDist then
        ...
        for i = #castQueue, 1, -1 do
            ...
            if skill:GetName() == "lina_light_strike_array" then
                if enemy:HasModifier("modifier_eul_cyclone_datadriven") then
                    gHeroVar.HeroPushUseAbilityOnLocation(bot, skill, enemy:GetAbsOrigin())
                    bot:ActionPush_Delay(modifiers.GetModifierRemainingDuration(enemy, "modifier_eul_cyclone_datadriven") - .95)
                elseif utils.IsCrowdControlled(enemy) then
                    gHeroVar.HeroPushUseAbilityOnLocation(bot, skill, enemy:GetAbsOrigin())
                else
                    gHeroVar.HeroPushUseAbilityOnLocation(bot, skill, GetExtrapolatedLocation(enemy, 0.95))
                end
            elseif skill:GetName() == "lina_dragon_slave" then
                if enemy:HasModifier("modifier_eul_cyclone_datadriven") then
                    gHeroVar.HeroPushUseAbilityOnLocation(bot, skill, enemy:GetAbsOrigin())
                    bot:ActionPush_Delay(modifiers.GetModifierRemainingDuration(enemy, "modifier_eul_cyclone_datadriven") - (0.45 + dist/1200))
                elseif utils.IsCrowdControlled(enemy) then
                    gHeroVar.HeroPushUseAbilityOnLocation(bot, skill, enemy:GetAbsOrigin())
                else
                    -- account for 0.45 cast point and speed of wave (1200) needed to travel the distance between us
                    gHeroVar.HeroPushUseAbilityOnLocation(bot, skill, GetExtrapolatedLocation(enemy, 0.45 + dist/1200))
                end
...
function linaAbility:AbilityUsageThink(bot)
	...
	local castWDesire, castWLocation  = ConsiderW(bot)
...
function ConsiderQ(bot)
    ...
    local Radius    = abilityQ:GetSpecialValueFor( "dragon_slave_width_end" )
		...
		if utils.ValidTarget(WeakestEnemy) then
			if not utils.IsTargetMagicImmune(WeakestEnemy) and not utils.IsCrowdControlled(WeakestEnemy) then
				if HeroHealth <= GetActualIncomingDamage(WeakestEnemy, Damage, DAMAGE_TYPE_MAGICAL) then
					...
					return BOT_ACTION_DESIRE_HIGH, GetExtrapolatedLocation(WeakestEnemy, 0.45 + d/1200 + getHeroVar(bot, "AbilityDelay"))
				end
			end
		end
	end
	...
	if #tableNearbyAttackingAlliedHeroes >= 2 then
		local locationAoE = bot:FindAoELocation( true, true, bot:GetLocation(), CastRange, Radius, 0.45 + getHeroVar(bot, "AbilityDelay"), 0 )
	...
	if modeName == "roam" or modeName == "defendally" or modeName == "fight" then
		...
		if utils.ValidTarget(npcEnemy) then
			...
			if not utils.IsTargetMagicImmune(npcEnemy) and not utils.IsCrowdControlled(npcEnemy) and 
				...
				return BOT_ACTION_DESIRE_HIGH, GetExtrapolatedLocation(npcEnemy, 0.45 + d/1200 + getHeroVar(bot, "AbilityDelay"))
			end
		end
	end
    ...
    if modeName == "jungling" or modeName == "laning" then
        if ManaPerc > 0.4 then
            local locationAoE = bot:FindAoELocation( true, false, bot:GetLocation(), CastRange, Radius, 0.45 + getHeroVar(bot, "AbilityDelay"), Damage )
    ...
    if modeName == "pushlane" or (modeName == "defendlane" and ManaPerc > 0.4) then
        local locationAoE = bot:FindAoELocation( true, false, bot:GetLocation(), CastRange, Radius, 0.45 + getHeroVar(bot, "AbilityDelay"), 0 )
        if locationAoE.count >= 3 then
            return BOT_ACTION_DESIRE_MODERATE, locationAoE.targetloc
        end
...
function ConsiderW(bot)
    ...
    -- Get some of its values
    local Radius    = abilityW:GetSpecialValueFor( "light_strike_array_aoe" )
    ...
    for _, npcEnemy in pairs( enemies ) do
        if utils.ValidTarget(npcEnemy) and npcEnemy:IsChanneling() then
            if not utils.IsTargetMagicImmune( npcEnemy ) then
                return BOT_ACTION_DESIRE_HIGH, npcEnemy:GetAbsOrigin()
            end
        end
    end
	...
	if modeName ~= "retreat" or (modeName == "retreat" and bot.SelfRef:getCurrentModeValue() < BOT_MODE_DESIRE_VERYHIGH) then
		if utils.ValidTarget(WeakestEnemy) then
			if not utils.IsTargetMagicImmune(WeakestEnemy) and not utils.IsCrowdControlled(WeakestEnemy) then
				if HeroHealth <= GetActualIncomingDamage(WeakestEnemy, Damage, DAMAGE_TYPE_MAGICAL) then
					return BOT_ACTION_DESIRE_HIGH, GetExtrapolatedLocation(WeakestEnemy, 0.95 + getHeroVar(bot, "AbilityDelay"))
				end
			end
		end
	end
	...
	if modeName == "roam" or modeName == "defendally" or modeName == "fight" then
		...
		if utils.ValidTarget(npcEnemy) then
			if not utils.IsTargetMagicImmune(npcEnemy) and not utils.IsCrowdControlled(npcEnemy) and 
				...
				return BOT_ACTION_DESIRE_HIGH, GetExtrapolatedLocation(npcEnemy, 0.95 + getHeroVar(bot, "AbilityDelay"))
			end
		end
	end
    ...
    if modeName == "jungling" or modeName == "laning" then
        if ManaPerc > 0.4 then
            local locationAoE = bot:FindAoELocation( true, false, bot:GetLocation(), CastRange, Radius, 0.95 + getHeroVar(bot, "AbilityDelay"), Damage )
            if locationAoE.count >= 3 then
                return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc
            end
        end
    ...
    if modeName == "pushlane" or (modeName == "defendlane" and ManaPerc > 0.4) then
        local locationAoE = bot:FindAoELocation( true, false, bot:GetLocation(), CastRange, Radius, 0.95 + getHeroVar(bot, "AbilityDelay"), 0 )
        if locationAoE.count >= 3 then
            return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc
        end
    ...
    if modeName ~= "retreat" or (modeName == "retreat" and bot.SelfRef:getCurrentModeValue() < BOT_MODE_DESIRE_VERYHIGH) then
        ...
        if utils.ValidTarget(WeakestEnemy) then
            if DamageType == DAMAGE_TYPE_MAGICAL then
                if not utils.IsTargetMagicImmune(WeakestEnemy) and not utils.IsCrowdControlled(WeakestEnemy) then
                    local actDmg = GetActualIncomingDamage(WeakestEnemy, Damage, DAMAGE_TYPE_MAGICAL)
