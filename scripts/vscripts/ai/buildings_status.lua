...
local buildings = {
    ...
    {["ApiID"]=ANCIENT, ["Type"]=TYPE_ANCIENT} -- 18
}
...
local function Initialize(bot)
    ...
    for i, building in ipairs(buildings) do
        ...
        if building.Type == TYPE_TOWER then
            ...
            pos_radiant = GetTower(TEAM_RADIANT, building.ApiID):GetAbsOrigin()
            pos_dire = GetTower(TEAM_DIRE, building.ApiID):GetAbsOrigin()
            towers[#towers+1] = i
        elseif building.Type == TYPE_MELEE then
            health = GetBarracks(team, building.ApiID):GetMaxHealth()
            pos_radiant = GetBarracks(TEAM_RADIANT, building.ApiID):GetAbsOrigin()
            pos_dire = GetBarracks(TEAM_DIRE, building.ApiID):GetAbsOrigin()
        ...
        elseif building.Type == TYPE_SHRINE then
            ...
            pos_radiant = GetShrine(TEAM_RADIANT, building.ApiID):GetAbsOrigin()
            pos_dire = GetShrine(TEAM_DIRE, building.ApiID):GetAbsOrigin()
            shrines[#shrines+1] = i
        elseif building.Type == TYPE_ANCIENT then
            health = GetAncient(team):GetMaxHealth()
            pos_radiant = GetAncient(TEAM_RADIANT):GetAbsOrigin()
            pos_dire = GetAncient(TEAM_DIRE):GetAbsOrigin()
        end
...
function Update(bot, forceUpdate)
