...
function X:Think(bot)
    ...
    if #Towers == 0 and #Barracks == 0 then
        ...
        -- no structures, no ancient, push lane
            ...
            local enemyFrontier = bot:GetLaneFrontAmount(utils.GetOtherTeam(bot), getHeroVar(bot, "CurLane"), false)
            local myFrontier = bot:GetLaneFrontAmount(bot:etTeam(), getHeroVar(bot, "CurLane"), true)
            local frontier = Min(Min(1.0, enemyFrontier), myFrontier)
            local dest = GetLocationAlongLane(getHeroVar(bot, "CurLane"), frontier)
    ...
    if #Towers > 0 then
        ...
        -- else, tower is not attacking me
            ...
            for i = 1, 5, 1 do
                local hTowerTarget = U.GetLaneTowerAttackTarget(U.GetOtherTeam(bot), getHeroVar(bot, "CurLane"), i)
            ...
            -- there is no friendly creep nearby or it would be being attacked
                ...
                local enemyFrontier = bot:GetLaneFrontAmount(utils.GetOtherTeam(bot), getHeroVar(bot, "CurLane"), false)
                local myFrontier = bot:GetLaneFrontAmount(GetTeam(bot), getHeroVar(bot, "CurLane"), true)
                local frontier = Min(Min(1.0, enemyFrontier), myFrontier)
                local dest = GetLocationAlongLane(getHeroVar(bot, "CurLane"), frontier)
                ...
                local dist = GetUnitToUnitDistance(bot, hTower)
                local range = bot:Script_GetAttackRange()
...
function X:Desire(bot)
    ...
    if not utils.IsCore(bot) then
        ...
        if #nearAllies > 1 then
            for _, ally in pairs(nearAllies) do
                if not ally:IsIllusion() and not ally:IsControllableByAnyPlayer() and utils.IsCore(ally) then
    ...
    local nearbyETowers = gHeroVar.GetNearbyEnemyTowers(bot, Max(750, bot:Script_GetAttackRange()))
    ...
    local nearbyEBarracks = gHeroVar.GetNearbyEnemyBarracks(bot, Max(750, bot:Script_GetAttackRange()))
    ...
    if GetUnitToUnitDistance(GetAncient(utils.GetOtherTeam(bot)), bot) < 1200 then
        return BOT_MODE_DESIRE_HIGH - 0.01
    end
