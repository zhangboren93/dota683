...
function FindTarget(bot)
    ...
    local enemies = bot:GetUnitList(UNIT_LIST_ENEMY_HEROES) -- check all enemies
    local allies = bot:GetUnitList(UNIT_LIST_ALLIED_HEROES)
    local ratings = {}
    
    local botMS = GetCurrentMovementSpeed(bot)
    if utils.GetHeroName(bot) == "spirit_breaker" then
        botMS = bot:GetAbilityByName("spirit_breaker_charge_of_darkness"):GetSpecialValueFor("movement_speed")
    end
    ...
    for i, e in pairs(enemies) do
        ...
        r = r + UnitPosFactor * (1 - global_game_state.GetPositionBetweenBuildings(e, bot:GetTeam()))
        local hero_count = 0
    ...
    if (GetEstimatedDamageToTarget(bot, true, target, 5.0, DAMAGE_TYPE_ALL ) * (1 + 0.5*heroAmpFactor)) > target:GetHealth() then
        setHeroVar(bot, "RoamTarget", target)
...
function X:Think(bot)    
    ...
    local target = getHeroVar(bot, "RoamTarget")
...
function X:Desire(bot)
    if getHeroVar(bot, "Role") == ROLE_ROAMER or bot.SelfRef:IsReadyToGank(bot) then
        ...
        if utils.ValidTarget(roamTarget) then
            ...
            local timeToIntercept = dist/GetCurrentMovementSpeed(bot)
            local timeUntilEscaped = utils.TimeForEnemyToGetIntoTheirBase(bot, roamTarget)
