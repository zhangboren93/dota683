...
function GetSecretShop(bot)

    if bot:GetTeam() == TEAM_RADIANT then
        local safeTower = utils.GetLaneTower(utils.GetOtherTeam(bot), LANE_BOT, 1)

        if utils.NotNilOrDead(safeTower) then
            return constants.SECRET_SHOP_RADIANT
        end
    else
        local safeTower = utils.GetLaneTower(utils.GetOtherTeam(bot), LANE_TOP, 1)
...
local function BuyItem( bot, sItem )
    if bot:GetGold() >= GetItemCost( sItem ) then
        ...
        table.remove(getHeroVar(bot, "ItemPurchaseClass"):GetPurchaseOrder() , 1)
...
function X:OnEnd(bot)
    setHeroVar(bot, "ShopType", constants.SHOP_TYPE_NONE)
    setHeroVar(bot, "NextShopItem", nil)
end
...
function X:Think(bot)
    ...
    local bDone = false
    if getHeroVar(bot, "ShopType") == constants.SHOP_TYPE_SIDE then
        bDone = ThinkSideShop( getHeroVar(bot, "NextShopItem") )
    elseif getHeroVar(bot, "ShopType") == constants.SHOP_TYPE_SECRET then
        bDone = ThinkSecretShop( getHeroVar(bot, "NextShopItem") )
    else
...
function X:Desire(bot)
    ...
    local sNextItem = getHeroVar(bot, "ItemPurchaseClass"):GetPurchaseOrder()[1]
    setHeroVar(bot, "NextShopItem", sNextItem)
    ...
    if bInSide then
        setHeroVar(bot, "ShopType", constants.SHOP_TYPE_SIDE)
    ...
    elseif bInSecret then
        setHeroVar(bot, "ShopType", constants.SHOP_TYPE_SECRET)
        if GetItemCost( sNextItem ) >= 2000 then
            return BOT_MODE_DESIRE_VERYHIGH
        else
            return BOT_MODE_DESIRE_HIGH
