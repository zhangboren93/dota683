...
function X:Think(bot)
    if modifiers.IsRuptured(bot) then
        local tp = utils.IsItemAvailable(bot, "item_tpscroll")
        if tp then
            gHeroVar.HeroUseAbilityOnLocation(bot,  tp, utils.Fountain( bot:GetTeam() ) )
            return
        else
            utils.myPrint("FIXME: we are ruptured and don't have TP... need to implement")
            
            -- consider using abilities
            local bAbilityQueued = getHeroVar(bot, "AbilityUsageClass"):AbilityUsageThink(bot)
    ...
    if # linProjs > 0 then
        ...
        if projMinDist < 2500 then
            ...
            if h <= (projRadius + 150) then
                ...
                local speed = GetCurrentMovementSpeed(bot)
...
function X:Desire(bot)
    ...
    if #aoes > 0 then
        for _, aoe in pairs(aoes) do
            if GetUnitToLocationDistance(bot, aoe.location) < (aoe.radius+bot:BoundingRadius2D()) then
                return BOT_MODE_DESIRE_ABSOLUTE
            end
        end
    end
