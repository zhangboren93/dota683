...
local function GetDefendeesTarget(bot, range)
    local nearAllies = bot:GetUnitList(UNIT_LIST_ALLIED_HEROES)
    ...
    for _, ally in pairs(nearAllies) do
        if not ally:IsIllusion() and not ally:IsControllableByAnyPlayer() and gHeroVar.HasID(ally:GetPlayerID()) then
            ...
            (ally.SelfRef:getCurrentMode():GetName() == "retreat") then
        ...
        for _, enemy in pairs(enemies) do
            if utils.ValidTarget(enemy) then
                ...
                for _, ally in pairs(alliesNeedingHelp) do
                    if WasRecentlyDamagedByHero(ally, enemy, 3.0) then
                        numAlliesDamaged = numAlliesDamaged + 1
                    end
                end

                if numAlliesDamaged > 0 then
                    local dmg = GetEstimatedDamageToTarget(bot, true, enemy, 4.5, DAMAGE_TYPE_ALL)
