...
function X:InitTable(bot)
    -- Init tables based on role    
    if (getHeroVar(bot, "Role") == role.ROLE_MID ) and next(self.ItemsToBuyAsMid) ~= nil then
        ...
        return true
    elseif (getHeroVar(bot, "Role") == role.ROLE_HARDCARRY ) and next(self.ItemsToBuyAsHardCarry) ~= nil then
        ...
        return true
    elseif (getHeroVar(bot, "Role") == role.ROLE_OFFLANE ) and next(self.ItemsToBuyAsOfflane) ~= nil then
        ...
        return true
    elseif (getHeroVar(bot, "Role") == role.ROLE_HARDSUPPORT
        or getHeroVar(bot, "Role") == role.ROLE_SEMISUPPORT ) and next(self.ItemsToBuyAsSupport) ~= nil then
        ...
        return true
    elseif (getHeroVar(bot, "Role") == role.ROLE_JUNGLER ) and next(self.ItemsToBuyAsJungler) ~= nil then
        ...
        return true
    elseif (getHeroVar(bot, "Role") == role.ROLE_ROAMER ) and next(self.ItemsToBuyAsRoamer) ~= nil then
	...
	if next(self.ItemsToBuyAsRoamer) ~= nil then
		self.setItemsFromTemplate(self.ItemsToBuyAsRoamer)
        return true
	elseif next(self.ItemsToBuyAsJungler) ~= nil then
		self:setItemsFromTemplate(self.ItemsToBuyAsJungler)
        return true
	elseif next(self.ItemsToBuyAsSupport) ~= nil then
		self:setItemsFromTemplate(self.ItemsToBuyAsSupport)
        return true
	elseif next(self.ItemsToBuyAsOfflane) ~= nil then
		self:setItemsFromTemplate(self.ItemsToBuyAsOfflane)
        return true
	elseif next(self.ItemsToBuyAsMid) ~= nil then
		self:setItemsFromTemplate(self.ItemsToBuyAsMid)
        return true
	elseif next(self.ItemsToBuyAsHardCarry) ~= nil then
		self:setItemsFromTemplate(self.ItemsToBuyAsHardCarry)
        return true
	end
end

function X:setItemsFromTemplate(template)
    self:SetStartingItems(template.StartingItems)
    self:SetUtilityItems(template.UtilityItems)
    self:SetCoreItems(template.CoreItems)
    self:SetExtensionItems(template.ExtensionItems)
    self:SetSellItems(template.SellItems)
end
-------------------------------------------------------------------------------
-- Buy functions
-------------------------------------------------------------------------------

function X:BuySupportItems(bot)
    ...
    --]]
        ...
        if tDelta > 10.0 then
            ...
            --local wards = GetItemStockCount(bot, "item_ward_observer")
...
function X:UpdatePurchaseOrder(bot)
...
function X:ConsiderBuyingExtensions(bot)
    ...
    if getHeroVar(bot, "HasMovementAbility") ~= nil then
        retreatAbility = true
    else
    ...
    if TrueStrikeCount > 0 then
        if utils.InTable(self.ExtensionItems.OffensiveItems, "item_butterfly_datadriven") then
            local ItemIndex = utils.PosInTable(self.ExtensionItems.DefensiveItems, "item_butterfly_datadriven")
    ...
    if DamagePhysical > DamageMagical then
        if utils.InTable(self.ExtensionItems.DefensiveItems, "item_hood_of_defiance_datadriven") or utils.InTable(self.ExtensionItems.DefensiveItems, "item_pipe") then
            --utils.myPrint(" Considering magic damage reduction")
        elseif utils.InTable(self.ExtensionItems.DefensiveItems, "item_black_king_bar_datadriven") then
            if retreatAbility and SilenceCount > 1 then
                --utils.myPrint(" Considering buying bkb")
            elseif SilenceCount > 2 or DamageTime > 8 then
                --utils.myPrint(" Considering buying bkb")
            else
                local ItemIndex = utils.PosInTable(self.ExtensionItems.DefensiveItems, "item_black_king_bar_datadriven")
                table.remove(self.ExtensionItems.DefensiveItems, ItemIndex)
