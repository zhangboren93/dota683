...
function sbBot:ConsiderAbilityUse(bot)
...
function ConsiderActionsWhileCharging(bot) 
    ...
    if not target then target = getHeroVar(bot, "Target") end
    ...
    if not utils.ValidTarget(target) then
        ...
        if GetUnitToLocationDistance(target, utils.Fountain(utils.GetOtherTeam(bot))) < 1250 then
            bot:Action_ClearActions(true)
            return
        end
    end
