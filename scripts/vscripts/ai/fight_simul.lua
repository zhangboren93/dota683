...
function considerAbility(ability, hTarget)
    ...
    if not ability:IsPassive() and ability:IsFullyCastable() and (not channelTime > 0) then
        ...
        return actualOneTimeCastDmg, ability:GetManaCost(-1), ability:GetCastPoint(), 0, ability:GetCooldown() --FIXME: Duration
    end
    ...
    if ability:IsFullyCastable() and channelTime > 0 then
        ...
        return actualOneTimeCastDmg, ability:GetManaCost(-1), ability:GetCastPoint(), channelTime, ability:GetCooldown()
    end
...
function getLinaDmg(duration, hero, target)
    ...
    while startTime < duration do
        ...
        totalDmg = totalDmg + GetActualIncomingDamage(target, rightClickDmg, DAMAGE_TYPE_PHYSICAL)
...
function estimateRightClickDamage( hHero, hTarget, fDuration )
    ...
    local actualDmg = GetActualIncomingDamage(hTarget, hHero:GetAttackDamage(), DAMAGE_TYPE_PHYSICAL)
    local numHits = math.floor(fDuration/hHero:GetSecondsPerAttack(false))
...
function estimateTimeToKill(hero, target)
    ...
    return hero:GetSecondsPerAttack(false)*(numHits-1) + rightClickCastPoint
end
