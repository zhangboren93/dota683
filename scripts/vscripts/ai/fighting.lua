    ...
    for _, enemy in ipairs(viableEnemiesToFight) do
        ...
        for _, ally in pairs(listAllies) do
            if ally:IsAlive() and (ally:GetHealth()/ally:GetMaxHealth()) > 0.4 then
                local timeToLocation = GetUnitToUnitDistance(enemy, ally)/GetCurrentMovementSpeed(ally)
...
function FindTarget(listEnemies, listEnemyTowers, listAlliedTowers, listEnemyCreeps, listAlliedCreeps)
	...
	for _, enemy in pairs(listEnemies) do
		if utils.NotNilOrDead(enemy) and GetUnitToLocationDistance(enemy, utils.Fountain(utils.GetOtherTeam())) > 1350 then
            ...
            local sd = GetStunDuration(bot, true) + 0.5*GetSlowDuration(bot, true)
            ...
            for _, ally in pairs(allyList) do
                if ally:GetPlayerID() ~= bot:GetPlayerID() then -- remove ourselves from consideration
                    ...
                    if utils.NotNilOrDead(Ally) and timeToLocation <= sd then
                        goodStunDuration = goodStunDuration + GetStunDuration(ally, true) + 0.5*GetSlowDuration(ally, true)
                    end
                end
			end
            ...
            for _, ally in pairs(allyList) do
                ...
                if timeToLocation < goodStunDuration then
                    boomDmg = boomDmg + GetEstimatedDamageToTarget(ally, true, enemy, goodStunDuration - timeToLocation, DAMAGE_TYPE_ALL)
                end
            end
				...
				if type(k) == "number" and enemy2.Alive and (enemy2.Health/enemy2.MaxHealth) > 0.1 then
					...
					if distance < 1200 then
                        ...
                        if utils.ValidTarget(enemy2) then
                            badDmg = badDmg + GetEstimatedDamageToTarget(enemy2.Obj, true, bot, dmgTime, DAMAGE_TYPE_ALL)
                        end
					end
				end
			end
            ...
            for _, ally in pairs(allyList) do
				...
				if utils.NotNilOrDead(Ally) and timeToLocation < goodFightLength then
					goodFightLength = goodFightLength + GetStunDuration(ally, true) + 0.5*GetSlowDuration(ally, true)
				end
			end
            ...
            for _, ally in pairs(allyList) do
				...
				if utils.NotNilOrDead(Ally) and timeToLocation < goodFightLength then
                    ...
                    goodDmg = goodDmg + GetEstimatedDamageToTarget(ally, true, enemy, dmgTime, DAMAGE_TYPE_ALL)
				end
			end
