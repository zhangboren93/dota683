...
function EnemyInfo.BuildEnemyList(bot)
    if GameTime() - LastEnemyUpdate > UpdateFreq1 then
        local listEnemyIDs = GetTeamPlayers(utils.GetOtherTeam(bot))
        ...
        for slot_id, pid in ipairs(listEnemyIDs) do
            ...
            -- can we see them now
            local hEnemy = EnemyInfo.GetEnemyHeroFromId(bot, pid)
            if hEnemy then
                ...
                if not hEnemy:IsNull() then
                    ...
                    EnemyInfo[pid].Location = hEnemy:GetAbsOrigin()
                    EnemyInfo[pid].LastSeen = GameTime()
                    EnemyInfo[pid].Lane = utils.NearestLane(bot:GetTeam(), hEnemy)
                    EnemyInfo[pid].ExtraLoc1 = GetExtrapolatedLocation(hEnemy, 1.0)
                    EnemyInfo[pid].ExtraLoc3 = GetExtrapolatedLocation(hEnemy, 3.0)
                    EnemyInfo[pid].ExtraLoc5 = GetExtrapolatedLocation(hEnemy, 5.0)
...
function EnemyInfo.InferRole(bot, id )
    local hEnemy = EnemyInfo.GetEnemyHeroFromId(bot, id)
    if hEnemy then
        if EnemyInfo[id].multiple then
            hEnemy = hEnemy[1]
        end
...
function EnemyInfo.GetEnemyHeroFromId( bot, id )
    local enemyList = bot:GetUnitList(UNIT_LIST_ENEMY_HEROES)
