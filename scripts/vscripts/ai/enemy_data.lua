...
function EnemyData.CheckAlive(bot)
    local enemyIDs = GetTeamPlayers(utils.GetOtherTeam(bot))
...
function EnemyData.UpdateEnemyInfo(bot, timeFreq)
    if ( GetGameState() ~= GAME_STATE_GAME_IN_PROGRESS and GetGameState() ~= GAME_STATE_PRE_GAME ) then return end
    
    EnemyData.CheckAlive(bot)
    ...
    for _, enemy in pairs(enemies) do
        ...
        if (GameTime() - EnemyData[pid].Time1) >= UpdateFreq1 then
            ...
            EnemyData[pid].MoveSpeed    = enemy:GetMoveSpeedModifier(enemy:GetBaseMoveSpeed(), false)
            ...
            if (GameTime() - EnemyData[pid].Time2) >= UpdateFreq2 then
                ...
                EnemyData[pid].SlowDur = GetSlowDuration(enemy, false) -- FIXME: does this count abilities only, or Items too?
                EnemyData[pid].StunDur = GetStunDuration(enemy, false) -- FIXME: does this count abilities only, or Items too?
                EnemyData[pid].HasSilence = HasSilence(enemy, false) -- FIXME: does this count abilities only, or Items too?
                EnemyData[pid].HasTruestrike = enemy:IsUnableToMiss()

                EnemyData[pid].AttackDamage = enemy:GetAttackDamage()
                EnemyData[pid].SecondsPerAttack = enemy:GetSecondsPerAttack(false)

                local allies = bot:GetUnitList(UNIT_LIST_ALLIED_HEROES)
                for _, ally in pairs(allies) do
					-- TODO
                    EnemyData[pid].PhysDmg2[ally:GetPlayerID()] = GetEstimatedDamageToTarget(enemy, true, ally, 2.0, DAMAGE_TYPE_PHYSICAL)
                    EnemyData[pid].MagicDmg2[ally:GetPlayerID()] = GetEstimatedDamageToTarget(enemy, true, ally, 2.0, DAMAGE_TYPE_MAGICAL)
                    EnemyData[pid].PureDmg2[ally:GetPlayerID()] = GetEstimatedDamageToTarget(enemy, true, ally, 2.0, DAMAGE_TYPE_PURE)
                    EnemyData[pid].AllDmg2[ally:GetPlayerID()] = GetEstimatedDamageToTarget(enemy, true, ally, 2.0, DAMAGE_TYPE_ALL)
                    EnemyData[pid].PhysDmg10[ally:GetPlayerID()] = GetEstimatedDamageToTarget(enemy, true, ally, 10.0, DAMAGE_TYPE_PHYSICAL)
                    EnemyData[pid].MagicDmg10[ally:GetPlayerID()] = GetEstimatedDamageToTarget(enemy, true, ally, 10.0, DAMAGE_TYPE_MAGICAL)
                    EnemyData[pid].PureDmg10[ally:GetPlayerID()] = GetEstimatedDamageToTarget(enemy, true, ally, 10.0, DAMAGE_TYPE_PURE)
                    EnemyData[pid].AllDmg10[ally:GetPlayerID()] = GetEstimatedDamageToTarget(enemy, true, ally, 10.0, DAMAGE_TYPE_ALL)
                end
            end
        end
    end
...
function EnemyData.GetEnemyTeamSlowDuration()
    local duration = 0
    for k, v in pairs(EnemyData) do
        if type(k) == "number" and v.SlowDur ~= nil then
...
function EnemyData.GetEnemyTeamStunDuration()
    local duration = 0
    for k, v in pairs(EnemyData) do
        if type(k) == "number" and v.StunDur ~= nil then
