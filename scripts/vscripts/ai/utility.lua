...
function U.GetHeightDiff(hUnit1, hUnit2)
    ...
    if type(hUnit2) == "number" then -- case for trees
        return (hUnit1:GetAbsOrigin().z - hUnit2)
    end
    
    if not U.ValidTarget(hUnit2) then return 0 end
    
    return (hUnit1:GetAbsOrigin().z - hUnit2:GetAbsOrigin().z)
end

function U.EnemyDistanceFromTheirAncient(bot, hEnemy )
    local locAncient = GetAncient(U.GetOtherTeam(bot)):GetAbsOrigin()
    if U.ValidTarget(hEnemy) then
        return U.GetDistance( locAncient, hEnemy:GetAbsOrigin() )
    end
    return 0
end

function U.TimeForEnemyToGetIntoTheirBase(bot, hEnemy )
    local distFromBase = U.EnemyDistanceFromTheirAncient(bot, hEnemy )
    return distFromBase/GetCurrentMovementSpeed(hEnemy)
end
...
function U.IsFacingLocation(hero, loc, delta)
    ...
    local moveVect = loc - hero:GetAbsOrigin()
...
function U.GetEnemyHeroesBetweenMeAndLoc(bot, loc, lineOfSightThickness)
    local fHeroList = {}
...
function U.IsCore( hHero )
    if hHero:IsControllableByAnyPlayer() then return true end
...
function U.IsMelee(hero)
    ...
    return hero:Script_GetAttackRange() < 320.0
end

function U.PartyChat(bot, msg)
...
function U.LevelUp(bot, AbilityPriority)
	...
	print(bot:GetName() .. " levels " .. AbilityPriority[1])
    local ability = bot:GetAbilityByName(AbilityPriority[1])

    if ( ability == nil ) then
        U.myPrint(bot:GetName() .. " FAILED AT Leveling " .. AbilityPriority[1] )
    ...
    if ( ability:CanAbilityBeUpgraded() and ability:GetLevel() < ability:GetMaxLevel() ) then
        ...
        U.myPrint(bot:GetName() .. " Leveling " .. ability:GetName() )
        table.remove( AbilityPriority, 1 )
    end
end

function U.GetOtherTeam(bot)
...
function U.PositionAlongLane(hUnit, lane)
    ...
    local fAmount = GetAmountAlongLane(hUnit:GetTeam(), lane, unitPos)
    local bInLane = false
    if fAmount.distance <= 1600 then
        bInLane = true
    end
    setHeroVar(hUnit, "IsInLane", bInLane)

    return fAmount.amount
end

function U.NearestLane(myTeam, hUnit)
    local unitPos = hUnit:GetAbsOrigin()
    for i = 1, 3, 1 do
        local fAmount = GetAmountAlongLane(myTeam, i, unitPos)
...
function U.IsInLane(bot)

    setHeroVar(bot, "RetreatLane", getHeroVar(bot, "CurLane"))
    setHeroVar(bot, "RetreatPos", getHeroVar(bot, "LanePos"))

    local minDis = 10000

    for i = 1, #U.Lanes, 1 do
        local lAmnt = U.PositionAlongLane(bot, U.Lanes[i])
        local thisDis = U.GetDistance(bot:GetLocationAlongLane(U.Lanes[i], lAmnt), bot:GetLocation())

        if thisDis < minDis then
            setHeroVar(bot, "RetreatLane", U.Lanes[i])
            setHeroVar(bot, "RetreatPos", lAmnt)
        end
    end

    return getHeroVar(bot, "IsInLane"), getHeroVar(bot, "RetreatLane")
end
...
function U.EnemiesNearLocation(bot, loc, dist)
    ...
    for _, enemy in pairs(listEnemies) do
        if U.ValidTarget(enemy) and U.GetDistance(enemy:GetAbsOrigin(), loc) <= dist then
            num = num + 1
        end
    end
...
function U.GetCenterOfCreeps(creeps)
    ...
    for _,creep in pairs(creeps) do
        if (string.find(creep:GetUnitName(),"melee")~=nil) then
            center = center + (creep:GetAbsOrigin())*meleeW;
            n=n+meleeW;
        else
            n=n+1;
            center = center + creep:GetAbsOrigin();
        end
    end
    if n==0 then
        return nil;
...
function U.IsHeroAttackingMe(hero, fTime)
    ...
    if WasRecentlyDamagedByHero(bot, hero, fTime) then
        return true
    end
    return false
end
...
function U.IsTowerAttackingMe(bot)
...
function U.CanHarass(bot, hTarget)
    ...
    local otherEnemiesInWay = U.GetEnemyHeroesBetweenMeAndLoc(bot, hTarget:GetAbsOrigin()+100, 500)
...
function U.DirectionAwayFromDanger(bot, listDangerHandles, origDir)
    local safeDir = origDir

    local otherEnemiesInWay = U.GetEnemyHeroesBetweenMeAndLoc(bot, U.VectorTowards(bot:GetLocation(), safeDir, 1600), 1000)
    if #otherEnemiesInWay == 0 then
        return safeDir
    end
...
function U.UseOrbEffect(bot)
    ...
    if orb then
        ...
        if ability ~= nil and ability:IsFullyCastable() then
            ...
            if U.ValidTarget(target) and GetUnitToUnitDistance(bot, target) < (bot:Script_GetAttackRange()+bot:BoundingRadius2D()) then
...
function U.InTeamFight(bot, range)
    ...
    local allyList = bot:GetUnitList(UNIT_LIST_ALLIED_HEROES)
    for _, ally in pairs(allyList) do
        if ally:IsAlive() and not ally:IsIllusion() and not ally:IsControllableByAnyPlayer() and
    ...
    for _, npcEnemy in pairs( gHeroVar.GetNearbyEnemies( bot, range ) ) do
        if not U.IsUnitCrowdControlled(npcEnemy) and not npcEnemy:IsInvulnerable() then
            if checkForMagicImmunity then
                local Damage = GetEstimatedDamageToTarget(npcEnemy, false, bot, 3.0, DAMAGE_TYPE_ALL )
...
function U.GetTeleportationAbility(bot)
    ...
    local tp = U.IsItemAvailable(bot, "item_tpscroll")
    if tp then
        return tp
    end

    tp = U.IsItemAvailable(bot, "item_travel_boots_datadriven")
    if tp then
        return tp
    end
...
function U.IsItemAvailable(bot, item_name)
