...
function handleAbilityExecuted(event)
	...
	local caster = event.caster
	if event_ability:GetName() == "troll_warlord_switch_stance" then
		if caster:HasModifier("modifier_troll_warlord_melee") then
			caster:SwapAbilities("troll_warlord_whirling_axes_ranged_datadriven", "troll_warlord_whirling_axes_melee_datadriven", true, false)
		else
			caster:SwapAbilities("troll_warlord_whirling_axes_melee_datadriven", "troll_warlord_whirling_axes_ranged_datadriven", true, false)
		end
	end
end
