...
function StartBrewing( event )
	...
	caster:FindAbilityByName(sub_ability_name):SetLevel(ability:GetLevel())
...
function CheckSelfStun( event )
	...
	if ability.time_charged >= brew_explosion then
		...
		ability:ApplyDataDrivenModifier(caster, caster, "modifier_stunned", {duration = max_stun})
...
function ConcoctionHit( event )
	...
	local heroes_around = { target }
	if caster ~= target then
		if target:TriggerSpellAbsorb(ability) then
			return
		end
		
		heroes_around = FindUnitsInRadius(
			caster:GetTeam(),
			target:GetAbsOrigin(),
			nil,
			ability:GetSpecialValueFor("midair_explosion_radius"),
