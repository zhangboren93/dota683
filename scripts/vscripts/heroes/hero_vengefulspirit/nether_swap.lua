...
function handleAbilityPhaseStart(event)
	...
	if not caster:HasScepter() and target:IsConsideredHero() then return end
	if caster:HasScepter() and (target:IsCreep() or target:IsConsideredHero()) then return end
	caster:Interrupt()
end
