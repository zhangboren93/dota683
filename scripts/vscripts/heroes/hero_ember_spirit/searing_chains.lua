...
function searing_chains_pin_point( keys )
	...
	local targetFlag = DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE
	...
	local shuffled_units = {}
	while #units > 0 do
		local random_id = RandomInt(1, #units)
		table.insert(shuffled_units, units[random_id])
		table.remove(units, random_id)
	end
	units = shuffled_units
	...
	for k, v in pairs( units ) do
		if count < maxTarget then
			...
			end, "Stop searing particle", duration)
