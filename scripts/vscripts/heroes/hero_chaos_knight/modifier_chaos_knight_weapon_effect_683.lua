modifier_chaos_knight_weapon_effect_683_lua = class({
	OnCreated = function(self, data)
		...
		if data.style ~= nil and parent ~= nil then
			...
			if data.style == "deft" then
				self.particleId = ParticleManager:CreateParticle("particles/units/heroes/hero_chaos_knight/chaos_knight_ambient_mace_default.vpcf", PATTACH_POINT_FOLLOW, parent)
			elseif data.style == "gold" then
				self.particleId = ParticleManager:CreateParticle("particles/units/heroes/hero_chaos_knight/chaos_knight_ambient_mace_gold.vpcf", PATTACH_POINT_FOLLOW, parent)
			end
