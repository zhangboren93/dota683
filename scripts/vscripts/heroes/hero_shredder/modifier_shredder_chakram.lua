...
modifier_shredder_chakram_move_lua = class({
	...
	OnIntervalThink = function(self)
		if not IsServer() then return end
		local parent = self:GetParent()
		parent:SetAbsOrigin(GetGroundPosition(parent:GetAbsOrigin() + self.velocity * 0.03, nil))
	end,
...
modifier_shredder_chakram_return_lua = class({
	...
	OnIntervalThink = function(self)
		...
		if self.damage_check_count % 3 == 0 then
		...
		self.damage_check_count = self.damage_check_count + 1
		if (parent:GetAbsOrigin() - destination):Length2D() < 50 then
			...
			if parent.particle_id ~= nil then
				ParticleManager:DestroyParticle(parent.particle_id, false)
				parent.particle_id = nil
			end
			parent:StopSound("Hero_Shredder.Chakram")
			caster:RemoveModifierByName("modifier_shredder_chakram_disarm_datadriven")
			parent:RemoveModifierByName("modifier_shredder_chakram_lua")
			parent:ForceKill(false)
		else
			velocity = destination - parent:GetAbsOrigin()
			velocity = Vector(velocity.x, velocity.y, 0):Normalized() * 27
			local new_destination = parent:GetAbsOrigin() + velocity
			parent:SetAbsOrigin(GetGroundPosition(new_destination, nil))
		end
	end
})
