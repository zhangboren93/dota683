...
function legion_commander_duel_datadriven_on_spell_start(keys)
	...
	local modifier_duel = "modifier_legion_commander_duel_datadriven"
...
function modifier_duel_datadriven_on_death(keys)
	...
	local ability = keys.ability
	local modifier_duel = "modifier_legion_commander_duel_datadriven"
	local modifier_duel_damage = "modifier_legion_commander_duel_damage_boost"
			...
			if individual_hero:GetTeam() ~= caster_team and individual_hero:HasModifier(modifier_duel) then
				if individual_hero:HasModifier(modifier_duel) then
					if not individual_hero:HasModifier(modifier_duel_damage) then
						individual_hero:AddNewModifier(caster, ability, modifier_duel_damage, {})
					end
					...
					local duel_victory_particle = ParticleManager:CreateParticle("particles/units/heroes/hero_legion_commander/legion_commander_duel_victory.vpcf", PATTACH_OVERHEAD_FOLLOW, individual_hero)
				end
			end
		end
