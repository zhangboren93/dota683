...
function shackleshot_hit( keys )
    ...
    if latch_target ~= nil then
		...
		if not target:TriggerSpellAbsorb(ability) then
			target:AddNewModifier(caster, ability, "modifier_stunned", { duration = stun_duration_long })
		end
        ...
        if latch_tree then
			...
			target:AddNewModifier(caster, ability, "modifier_stunned", { duration = stun_duration_fail })
