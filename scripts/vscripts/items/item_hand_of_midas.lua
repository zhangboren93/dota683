...
function item_hand_of_midas_datadriven_on_spell_start(keys)
	PlayerResource:ModifyGold(keys.caster:GetPlayerID(), keys.BonusGold, true, 0)  --Give the player a flat amount of reliable gold.
	...
	keys.target:ForceKill(false) --Kill the creep.  This increments the caster's last hit counter.
end